...
export default async function RootLayout({children}:{children:React.ReactNode}){const user=await currentUser();return <html lang="zh-CN"><body>
  <header className="site-header"><div className="container header-inner"><Link className="brand" href="/"><span className="brand-mark">未</span>未完</Link><nav className="nav" aria-label="主导航"><Link href="/">精选</Link><Link href="/submit">投稿</Link><Link className="claim-link keep" href="/claim"><span className="claim-label-long">认领文章</span><span className="claim-label-short">认领</span></Link>{user?.role==="admin"&&<AdminModeSwitch/>}{user?<><span className="account-name">{user.display_name}</span><form action={logout}><button className="link-button">退出</button></form></>:<Link className="button keep" href="/login">登录</Link>}</nav></div></header>
